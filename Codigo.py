...
lista_votantes = []
lista_asistencias = []
resultados_eleccion = pd.DataFrame() 

cedulas_registradas_jurados = set()
cedulas_registradas_votantes = set()
cedulas_registradas_asistencia = set()
...
def generar_estructura():
    global lista_jurados, cedulas_registradas_jurados
    datos = validar_entradas()
    if not datos:
        return
    salones, mesas, jurados = datos
    lista_jurados = [[[] for _ in range(mesas)] for _ in range(salones)]
    cedulas_registradas_jurados.clear()
...
def registrar_jurado(salon, mesa, jurado):
    ...
    def guardar():
        ...
        if "" in datos:
            ...
            return
        if datos[1] in cedulas_registradas_jurados:
            messagebox.showerror("Error", "La cédula ya está registrada para un jurado.")
            return
        lista_jurados[salon][mesa].append(datos)
        cedulas_registradas_jurados.add(datos[1])
...
def mostrar_mesa(salon, mesa):
    texto = ""
    lista_jurados = lista_jurados[salon][mesa]
    if lista_jurados:
        texto += "Jurados:\n"
    ...
    lista_votantes = [v for v in lista_votantes if v[2] == f"Salón {salon+1}" and v[3] == f"Mesa {mesa+1}"]
...
def guardar_jurados():
    try:
        with open("estructura.txt", "w") as f:
            f.write(str(lista_jurados))
        messagebox.showinfo("Guardado", "Datos guardados en estructura.txt")
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo guardar: {e}")

def cargar_votantes():
    global lista_votantes, cedulas_registradas_votantes
    ...
    try:
        with open(archivo, newline='') as csvfile:
            ...
            lista_votantes.clear()
            cedulas_registradas_votantes.clear()
            for row in lector:
                if row['cedula'] in cedulas_registradas_votantes:
                    messagebox.showerror("Error", f"Cédula duplicada en votantes: {row['cedula']}")
                    return
                cedulas_registradas_votantes.add(row['cedula'])
                lista_votantes.append([row['nombre'], row['cedula'], row['salon'], row['mesa']])
...
def buscar_cedula():
    ...
    for s in range(len(lista_jurados)):
        for m in range(len(lista_jurados[s])):
            for j in lista_jurados[s][m]:
                if j[1] == cedula:
...
def registrar_asistencia():
    ...
    def guardar():
        ...
        try:
            ...
            return
        if cedula in cedulas_registradas_asistencia:
            messagebox.showerror("Error", "Esta cédula ya registró asistencia.")
            return
        cedulas_registradas_asistencia.add(cedula)
        lista_asistencias.append(datos)
...
def cargar_resultados_json():
    ...
    try:
        ...
        resultados_eleccion = pd.DataFrame(datos)
        # Validar que tenga columnas correctas
        if not all(c in resultados_eleccion.columns for c in ["candidato", "votos"]):
...
def mostrar_estadisticas():
    if resultados_eleccion is None or resultados_eleccion.empty:
    ...
    text.pack()
    text.insert(tk.END, resultados_eleccion.to_string(index=False))

    #  gráfico de barras
    plt.figure(figsize=(6,4))
    plt.bar(resultados_eleccion['candidato'], resultados_eleccion['votos'], color='skyblue')
...
tk.Button(root, text="Guardar Datos Jurados", command=guardar_jurados).pack(pady=5)
